...
#define SECRET_SSID "ACT102541158955"    // replace MySSID with your WiFi network name
#define SECRET_PASS "68594361"  // replace MyPassword with your WiFi password
...
void displayData()
{
  ...
  if(((millis()-lastTimeH>timerDelayH)&(WiFi.status()==WL_CONNECTED))|healthBool){
    ...
    WiFiClient client;
    HTTPClient http;
    healthUpdate = serverName2+String("/empUpdate?empID=1NH19EC010")+String("&bpm=")+String(heartrate)+String("&temp=")+String(mlx.readObjectTempF())+String("&bo=")+String(spo2);
  ...
  display.println(String(heartrate)+"        |"+String(mlx.readAmbientTempF())+"F");
 ...
 display.print(" |"+String(mlx.readObjectTempF())+"F");
